...
for dataset in dataset_list:
    ...
    for iter in range(args.subset_train_iter):
        ...
        # Run the model
        best_acc, _ = run_experiment_mixup_approach(trainloader, testloader, subset_dataset, args.ns_epochs, net, optimizer, scheduler, best_acc, criterion, device, args.lr, args.alpha)
        print(best_acc)
    ...
    for iter in range(args.subset_train_iter):
        ...
        # Run the model
        best_acc, _ = run_experiment_mixup_approach(trainloader, testloader, dataset.split("/")[-1], args.epochs, net, optimizer, scheduler, best_acc, criterion, device, args.lr, args.alpha)
    ...
    for iteration in range(args.iterations):
        for trial in range(args.trials):
            ...
            for approach in args.approach_list:
                ...
                run_experiment_mixup_approach(trainloader, testloader, current_exp, args.epochs, net, optimizer, scheduler, best_acc, criterion, device, args.lr, args.alpha)
                ...
                _, metrics = run_experiment_mixup_approach(trainloader, testloader, current_exp, epochs_for_transfer_learning, net, optimizer, scheduler, best_acc, criterion, device, args.lr, args.alpha, iteration = iteration, trial = trial, dataset = dataset, classes = testset.classes, current_dataset_file = current_dataset_file)
