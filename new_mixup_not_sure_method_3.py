'''
Method 2 uses not-sure information to create a sure and not-sure class
It pretrains the model first on the sure and not-sure class, then
finetunes to the downstream task of classifying the original dataset classes
'''
...
for dataset in dataset_list:
    ...
    if create_sure_class:
        # Initial clean-up - Delete Sure class if already exists
        sure_train_dir = dataset + "/train/Sure/"
        sure_test_dir = dataset + "/test/Sure/"
        delete_dir_if_exists(sure_train_dir)
        delete_dir_if_exists(sure_test_dir)
    ...
    for approach in args.approach_list:
        ...
        if create_sure_class:
            sure_approach_dir = dataset + f"/approach_{approach}/Sure/"
            create_dir(sure_approach_dir, True)

    # Number of not-sure images to create - Total number of images in all class
    NS_ALL = int(len(glob.glob(dataset + "/train/*/*")) / 2)
    ...
    for iter in range(args.subset_train_iter):
        ...
        # Run the model
        best_acc, _ = run_experiment_mixup_approach(trainloader, testloader, subset_dataset, args.ns_epochs, net, optimizer, scheduler, best_acc, criterion, device, args.lr)
    ...
    for iter in range(args.subset_train_iter):
        ...
        # Run the model
        best_acc, _ = run_experiment_mixup_approach(trainloader, testloader, dataset.split("/")[-1], args.epochs, net, optimizer, scheduler, best_acc, criterion, device, args.lr)
    ...
    cam = None #construct_cam(net.module, [net.module.layer4[-1]], torch.cuda.is_available())
        ...
        if fp_total:
            ...
            for c_hat in list_c:
                ...
                if len(images_from_c_hat) > 0 and len(images_from_c_hat) == len(images_from_c):
                    mix(cam, images_from_c, images_from_c_hat, args.image_size, dataset, 0.0, 0.5, args.approach_list, class_c = c, class_c_hat = c_hat, add_class_name_to_naming = True)

                    # To generate images from the same class, shuffle so images created are not from the same images
                    folder_to_save_imgs = "Sure"

                    # Create another set of images from c so we can mix the two lists when creating images for the sure class
                    second_version_of_images_from_c = images_from_c.copy()

                    # Shuffle images from c so the same images are not mixed together
                    random.shuffle(images_from_c)
                    random.shuffle(second_version_of_images_from_c)
                    mix(cam, images_from_c, second_version_of_images_from_c, args.image_size, dataset, 0.0, 0.5, args.approach_list, sure_or_not_sure_folder = folder_to_save_imgs, class_c = c, class_c_hat = c_hat, add_class_name_to_naming = True)
    ...
    for iteration in range(args.iterations):
        for trial in range(args.trials):
            ...
            for approach in args.approach_list:
                ...
                # This for loop creates a new folder to serve as the binary dataset for sure and not-sure classification
                sure_not_sure_dataset_folder = 'Sure_Not_Sure'
                folders_for_sure_and_not_sure = ['Sure', 'Not_Sure'] # The classes in the binary dataset

                for folder in folders_for_sure_and_not_sure:

                    # Create dataset directory for training images for the current folder class
                    train_data_folder = f"{sure_not_sure_dataset_folder}/train/{folder}"
                    create_dir(train_data_folder, True)

                    # Create dataset directory for test images for the current folder class
                    test_data_folder = f"{sure_not_sure_dataset_folder}/test/{folder}"
                    create_dir(test_data_folder, True)

                    # Get all of the data of the current approach to be split into train and test
                    app_data = dataset + f"/approach_{approach}/{folder}/"
                    app_data_list = glob.glob(app_data + '*')
                    random.shuffle(app_data_list) # Randomize

                    # Get 90% of the approach data into train
                    for filename in app_data_list[:int(0.9 * len(app_data_list))]:
                        shutil.copy(filename, train_data_folder)

                    # Get 10% of the approach data into test
                    for filename in app_data_list[int(0.9 * len(app_data_list)):]:
                ...
                best_loss = float('inf')
                run_experiment_for_multilabels(trainloader, testloader, current_exp, args.epochs, net, optimizer, scheduler, best_loss, criterion, device, args.lr)
                ...
                delete_dir_if_exists(sure_not_sure_dataset_folder)

                best_acc = 0  # best test accuracy
                epochs_for_transfer_learning = args.epochs
                _, metrics = run_experiment_mixup_approach(trainloader, testloader, current_exp, epochs_for_transfer_learning, net, optimizer, scheduler, best_acc, criterion, device, args.lr, iteration = iteration, trial = trial, dataset = dataset, classes = testset.classes, current_dataset_file = current_dataset_file)
