...
for dataset in dataset_list:
    ...
    for c in range(no_of_classes):
        ...
        if not len(corr_preds_as_c_baseline) or not len(corr_preds_as_c):
            print("Continue line 165")
            continue
        else:
            print("Continue line 168")
        ...
        print(f"corr_preds_as_c_baseline: {len(corr_preds_as_c_baseline)}")

        # Compute uniform false positive error UFPE_c for class c
        UFPE_c  = fp_c  / ( no_of_classes - 1 )
        print(f"UFPE_c: {UFPE_c}")
        ...
        if fp_total:
            ...
            for c_hat in list_c:
                ...
                print(f"wrong_pred_in_c_hat: {len(wrong_pred_in_c_hat)}")
                fp_from_c_hat = len(file_pred_list[c_hat][c])
                print(f"fp_from_c_hat: {fp_from_c_hat}")

                # If any of the items are empty
                if not fp_from_c_hat or not len(wrong_pred_in_c_hat):
                    print("Items on line 198 are empty")
                    continue
                else:
                    print("Items on line 198 are NOT empty")
                ...
                print(f"Number of images from c: {len(images_from_c)}")
                print(f"Number of images from c_hat: {len(images_from_c_hat)}")
                ...
                if len(images_from_c_hat) > 0 and len(images_from_c_hat) == len(images_from_c):
                    ...
                    print("Mixing images here !!!")
                else:
                    print("Not mixing images at this time $$$")
